  ...
  async execute(args: any): Promise<void> {
    ...
    if (!args.file) {
      ...
      if (!existsSync(args.file)) {
        console.log(
          chalk.yellow(
            `Default file 'rulesync.mdc' not found. You can create it with 'onlyrules init <template-name>'`
          )
        );
        console.log(
          chalk.yellow(
            'Usage: onlyrules generate --file <path_or_url> [--output <dir>] [--verbose] [--force] [--no-ide-style] [--ide-folder <name>] [--traditional]'
          )
        );
        process.exit(1);
      }
    ...
    if (args.target) {
      ...
      try {
        ...
        console.log(
          chalk.green(`${configAction} onlyrules.json with targets: ${targetArray.join(', ')}`)
        );
      } catch (error) {
        console.warn(
          chalk.yellow(`Warning: Failed to update onlyrules.json: ${(error as Error).message}`)
        );
      }
