...
export async function generateRules(options: RuleGenerationOptions): Promise<void> {
  ...
  if (useNewArchitecture) {
    try {
      ...
      console.warn(
        chalk.yellow('⚠ New architecture failed, falling back to legacy implementation')
      );
      ...
      for (const format of formatsToGenerate) {
        ...
        for (const rule of rules) {
          ...
          try {
            ...
            const isRoot =
              rule.name === 'default' || rule.name === 'root' || rule.name === 'global';
            ...
            if (format === RuleFormat.CLAUDE_ROOT || format === RuleFormat.GEMINI_ROOT) {
              ...
              if (
                ...
                (rules.indexOf(rule) === 0 &&
                  !rules.some(
                    (r) => r.name === 'default' || r.name === 'root' || r.name === 'global'
                  ))
              ) {
              ...
              format === RuleFormat.CLAUDE_MEMORIES ||
              format === RuleFormat.GEMINI_MEMORIES
            ) {
              // Only generate memory files for non-root rules
              if (isSpecificFile) {
                await writeRulesToFile(
                  format,
                  rule.content,
                  options.output,
                  options.force,
      ...
      allResults.forEach((result) => {
        if (result.status === 'fulfilled') {
          ...
          if (value.success) {
            ...
            console.log(
              chalk.red(
                `✗ Failed to generate ${value.format} for rule '${value.ruleName}': ${value.error}`
              )
            );
          }
