...
- Use enums for related constants`;
    ...
    assert(
      appendedContent.includes('---\n\n# TypeScript Rules'),
      'Additional rules should be appended with section separator'
    );
    assert(
      appendedContent.length > initialContent.length,
    ...
    execSync(
      `node ${path.join(originalCwd, 'dist/cli.js')} generate -f ./rulesync.mdc --target cursor`,
      {
        stdio: 'inherit',
      }
    );
    ...
    for (const fileName of expectedFiles) {
      ...
      assert(
        fileContent.includes('cursorRuleType:'),
        `${fileName} should contain Cursor-specific frontmatter`
      );

      if (fileName === 'global.mdc') {
        assert(
          fileContent.includes('Basic AI Rules') || fileContent.includes('General Instructions'),
          'global.mdc should contain basic rules content'
        );
      } else if (fileName === 'typescript.mdc') {
        assert(
          fileContent.includes('TypeScript Rules') && fileContent.includes('Type Safety'),
    ...
    assert(
      ...
      'Should maintain section separators'
    );
...
async function testTraditionalStyle() {
  ...
  execSync(
    `node ./dist/cli.js generate --file ./rulesync.mdc --output ${traditionalDir} --no-ide-style`,
    {
      stdio: 'inherit',
    }
  );
  ...
  if (globalMdFile) {
    ...
    assert(
      globalContent.includes('Basic AI Rules') || globalContent.includes('General Instructions'),
    ...
    assert(
      globalAugment.includes('Basic AI Rules') || globalAugment.includes('General Instructions'),
      'Global rule should contain content from the global rule'
    );
  }
  ...
  if (stylesheetMdFile) {
    ...
    assert(
      stylesheetContent.includes('Content Guidelines'),
    ...
    assert(
      stylesheetAugment.includes('Content Guidelines'),
      'Stylesheet rule should contain content from the stylesheet rule'
    );
  }
...
async function testIdeStyle() {
  ...
  execSync(
    `node ./dist/cli.js generate --file ./rulesync.mdc --output ${ideStyleDir} --ide-folder ${customFolderName}`,
    {
      stdio: 'inherit',
    }
  );
