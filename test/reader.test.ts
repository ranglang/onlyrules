...
describe('reader utilities', () => {
  ...
  describe('isUrl', () => {
    ...
    it('should return true for valid HTTPS URLs', () => {
      ...
      expect(isUrl('https://onlyrules.codes/api/rules/raw?id=cmd9nww9z0007l5040oegtmb1')).toBe(
        true
      );
    });
  ...
  describe('readRulesFromInput', () => {
    ...
    it('should call readRulesFromFile for local file inputs', async () => {
      ...
      const mockReadFile = readFile as unknown as {
    ...
    it('should handle URL fetch errors', async () => {
      ...
      await expect(readRulesFromInput('https://example.com/nonexistent.md')).rejects.toThrow(
        'Failed to fetch rules from URL: 404 Not Found'
      );
    });
    ...
    it('should handle file read errors', async () => {
      ...
      await expect(readRulesFromInput('./nonexistent.md')).rejects.toThrow(
        'Error reading rules file: File not found'
      );
    });
  });
  ...
  describe('readRulesFromUrl', () => {
    ...
    it('should handle HTTP errors', async () => {
      ...
      await expect(readRulesFromUrl('https://example.com/error.md')).rejects.toThrow(
        'Error fetching rules from URL: Failed to fetch rules from URL: 500 Internal Server Error'
      );
    });
    ...
    it('should handle network errors', async () => {
      ...
      await expect(readRulesFromUrl('https://example.com/network-error.md')).rejects.toThrow(
        'Error fetching rules from URL: Network error'
      );
    });
  });
  ...
  describe('readRulesFromFile', () => {
    ...
    it('should handle file system errors', async () => {
      ...
      await expect(readRulesFromFile('./protected.md')).rejects.toThrow(
        'Error reading rules file: Permission denied'
      );
    });
  });
});
